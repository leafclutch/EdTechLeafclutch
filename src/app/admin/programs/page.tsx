...
export default function AdminProgramsPage() {
  ...
  async function togglePublish(id: string, current: boolean) {
    await supabase
      ...
      .eq("id", id);
    setToast(current ? "Program unpublished" : "Program published");
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        >
          <i className="fas fa-plus" /> Add Program
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-16 text-center border border-(--color-border)">
          ...
          >
            <i className="fas fa-plus" /> Create First Program
          </Link>
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="bg-white rounded-xl border border-(--color-border) overflow-hidden shadow-sm">
                  ...
                  >
                    <td className="pl-5 pr-3 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-(--color-primary)/8 flex items-center justify-center text-(--color-primary)">
                          <i className={`${program.icon} text-[15px]`} />
                        </div>
                        <div>
                    ...
                    <td className="px-3 py-4">
                      <div className="text-[14px] font-semibold text-(--color-dark)">
                        NPR {program.price.toLocaleString()}
                      </div>
                      <div className="text-[11px] text-(--color-text-light)">
                        Initial: NPR {program.initial_fee.toLocaleString()}
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-4 text-center">
                      <button
                        onClick={() =>
                          togglePublish(program.id, program.is_published)
                        }
                        className={`text-[11px] font-bold px-3 py-1 rounded-full transition-all ${program.is_published ? "bg-emerald-100 text-emerald-700 hover:bg-emerald-200" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
                      >
                        <i
                          className={`fas ${program.is_published ? "fa-eye" : "fa-eye-slash"} mr-1 text-[9px]`}
                        />
          ...
          <div className="px-5 py-3 border-t border-(--color-border) bg-gray-50/50 text-[12px] text-(--color-text-light)">
            Showing {filtered.length} of {programs.length} program
            {programs.length !== 1 ? "s" : ""}
          </div>
        </div>
      )}
